...
class LoginRecord(Base):
    async def list_login_record(self, creator: int, offset: int, limit: int) -> list:
    ...
    async def count_login_record(self, creator: int) -> list:
        record = await self.exec("count_login_record", creator=creator)
        if not record or not record[0] or record[0][0] == 0:
            return 0
    ...
    async def add_login_record(self, creator: int, host: str, type: int, token: str) -> int:
