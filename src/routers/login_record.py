...
async def add_login_record(
    ...
    record = await login_record_crud.add_login_record(uid=user.id,
...
async def list_login_record(
    ...
    login_record_crud = LoginRecord(request.app.state.pgpool)
    records, count = await login_record_crud.list_login_record(uid=user.id, offset=offset, limit=pagesize)
