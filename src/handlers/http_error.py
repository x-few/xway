...
async def server(request: Request, exc: HttpServerError) -> JSONResponse:
    traceback.print_exc()
    return JSONResponse({"detail": exc.detail}, status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)

async def client(request: Request, exc: HttpClientError) -> JSONResponse:
    return JSONResponse({"detail": exc.detail}, status_code=status.HTTP_400_BAD_REQUEST)

async def forbidden(request: Request, exc: HttpForbidden) -> JSONResponse:
    return JSONResponse({"detail": exc.detail}, status_code=status.HTTP_403_FORBIDDEN)

async def notfound(request: Request, exc: HttpNotFound) -> JSONResponse:
    return JSONResponse({"detail": exc.detail}, status_code=status.HTTP_404_NOT_FOUND)
